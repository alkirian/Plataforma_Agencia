...
export class SupabaseDocumentRepository extends DocumentRepository {
  ...
  async findByCriteria(criteria) {
    ...
    if (criteria.type) {
      ...
      if (mimePatterns.length > 0) {
        query = query.or(mimePatterns.map(pattern => `mime_type.like.${pattern}`).join(','));
      }
    }
    ...
    const documents = (hasMore ? data.slice(0, -1) : data).map(row => new DocumentEntity(row));
  ...
  entityToRow(entity) {
    return {
      ...
      filename_original: entity.filenameOriginal,
  ...
  getMimePatterns(type) {
    const patterns = {
      ...
      text: ['text/%'],
      compressed: ['application/zip', 'application/x-rar-compressed', 'application/x-tar', 'application/gzip'],
      design: ['application/x-photoshop', 'image/vnd.adobe.photoshop', 'application/postscript']
    };
