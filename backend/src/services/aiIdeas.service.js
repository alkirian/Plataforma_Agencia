...
const buildCandidateDates = ({ startDate, count, preferWeekdays, allowedWeekdays, month, year }) => {
  const candidates = [];
  const now = new Date();
  ...
  if (Array.isArray(allowedWeekdays) && allowedWeekdays.length) {
    allowedSet = new Set(allowedWeekdays.map(weekdayToJs).filter((x) => x !== null));
  ...
  for (let i = 0; i < limitDays && candidates.length < count * 4; i++) {
    ...
    if (d < now && (!month || !year || (month === now.getMonth() + 1 && year === now.getFullYear()))) continue;
    pushIfValid(d);
  }
  return candidates;
};
...
export const generateScheduleIdeas = async ({
  ...
  if (!openaiApiKey) {
    ...
    const cnd = buildCandidateDates({ startDate, count, preferWeekdays, allowedWeekdays, month, year });
    const picks = distributeDates({ candidates: cnd, count });
    const fallbackIdeas = Array.from({ length: Math.max(1, Number(count) || 10) }, (_, i) => {
      const d = picks[i] || new Date(now.getFullYear(), now.getMonth(), now.getDate() + i + 1);
      ...
      return {
        ...
        media: { type: 'carrusel', description: 'Carrusel de 5 diapositivas con tips clave.' },
        platforms,
  ...
  const baseQuery = userPrompt?.trim() ? userPrompt.trim() : `Genera ideas de calendario de contenido mensual para ${client.name}`;
  ...
  let matches = [];
  if (queryEmbedding && Array.isArray(queryEmbedding) && queryEmbedding.length) {
    try {
  ...
  const topContext = (matches || [])
    .map((m) => `${m.document_source_type?.toUpperCase() || 'CONTEXT'} ${m.document_name}: ${m.content}`)
    .join('\n---\n');
  ...
  const targetMonthName = new Date(targetYear, targetMonth - 1, 1).toLocaleString('es-ES', { month: 'long', year: 'numeric' });
  const nextTargetDate = new Date(targetYear, targetMonth, 1);
  const nextTargetMonthName = nextTargetDate.toLocaleString('es-ES', { month: 'long', year: 'numeric' });
  ...
  const megaPrompt = `Eres estratega de contenidos senior para redes sociales.
...
TAREA: Genera ${countSafe} ideas de posteos creativos para el cliente ${client.name}${userPrompt?.trim() ? `, enfocadas en: "${userPrompt.trim()}"` : ''}.
...
3. ${toneInstruction}
4. El copy en español, claro y accionable; máximo ~180 palabras. Evita clichés.
5. media.type concreto (ej: carrusel 5 slides, reel 30s, ilustración, influencer hablando, tutorial, behind-the-scenes) con description breve y accionable.
6. hashtags: de marca y tema, sin excesos; 3–6.
7. platforms: si no se especifican, sugiere según formato (p.ej., IG, TikTok, YouTube Shorts).
...
FORMATO DE RESPUESTA: Entrega SOLO un JSON válido: un array con objetos de la forma:
{
  ...
  const startDate = new Date((year || now.getFullYear()), (month ? month - 1 : now.getMonth()), 1);
  const cnd = buildCandidateDates({ startDate, count: countSafe, preferWeekdays, allowedWeekdays, month: month || (now.getMonth() + 1), year: year || now.getFullYear() });
  const picks = distributeDates({ candidates: cnd, count: countSafe });

  const safeIdeas = (Array.isArray(ideas) ? ideas : []).slice(0, countSafe).map((raw, i) => {
    const d = (() => {
    ...
    return {
      ...
      media: {
        type: String(raw?.media?.type || 'carrusel'),
        description: String(raw?.media?.description || ''),
      },
      platforms: Array.isArray(raw?.platforms) && raw.platforms.length ? raw.platforms : platforms,
      scheduled_at: `${y}-${m}-${day}`,
      status: 'Pendiente',
      tone,
    };
  ...
  try {
    const records = safeIdeas.map((idea, idx) => ({
      ...
      media_type: idea.media?.type || null,
      media_description: idea.media?.description || null,
    ...
    if (!error && Array.isArray(inserted)) {
      ...
      return safeIdeas.map((idea, idx) => ({ ...idea, id: idBySeq.get(idx), session_id: sessionId }));
    }
  } catch (_e) {
    // ignore
  }
...
export const upsertIdeaFeedback = async ({ clientId, ideaId, userId, token, value }) => {
  const supabaseAuth = createAuthenticatedClient(token);
  ...
  if (idea.client_id !== clientId) throw new Error('Idea no pertenece al cliente');

  if (value === 'clear') {
    await supabaseAuth.from('ai_idea_feedback')
      .delete()
      .eq('idea_id', ideaId)
      .eq('user_id', userId);
  } else {
  ...
  return { idea_id: ideaId, like_count: likeCount, dislike_count: dislikeCount };
};

export const listIdeas = async ({ clientId, month = null, year = null, sessionId = null, token }) => {
  const supabaseAuth = createAuthenticatedClient(token);
  let q = supabaseAuth
    .from('ai_ideas')
    .select('*')
  ...
  if (month && year) {
    ...
    q = q.gte('suggested_date', start.toISOString().slice(0, 10))
  ...
  return (data || []).map(r => ({
    ...
    media: { type: r.media_type || '', description: r.media_description || '' },
