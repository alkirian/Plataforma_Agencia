...
export const createInvitations = async ({ inviterId, inviterName, agencyId, emails, role = 'member', redirectUrl }) => {
  const errors = [];
  const invitations = [];
  ...
  for (const rawEmail of emails) {
    ...
    const frontendBase = process.env.FRONTEND_URL || redirectUrl || 'http://localhost:5173';
...
export const validateInvitationToken = async (token) => {
  const { data: invite, error } = await supabaseAdmin
    .from('invitations')
    .select('id, token, email, role, status, expires_at, agency:agencies(id, name), inviter:profiles(id, full_name, email)')
    .eq('token', token)
    .maybeSingle();
...
export const acceptInvitation = async ({ token, fullName, role, avatarUrl }) => {
  ...
  let userId = null;
  try {
    const { data: invited, error: invErr } = await supabaseAdmin.auth.admin.inviteUserByEmail(invite.email, {
  ...
  if (userId) {
    ...
    const { error: upErr } = await supabaseAdmin.from('profiles').upsert(updates);
  ...
  const { error: updErr } = await supabaseAdmin
    ...
    .eq('id', invite.id);
  if (updErr) throw new Error(`Error actualizando invitación: ${updErr.message}`);
...
export const resendInvitation = async ({ invitationId, agencyId }) => {
  ...
  if (error) throw new Error(error.message);
  if (!invite) throw new Error('Invitación no encontrada');
  if (invite.status !== 'pending') throw new Error('La invitación no está pendiente');
  if (new Date(invite.expires_at) < new Date()) throw new Error('Invitación expirada');
