...
router.route('/')
  ...
  .post(
    validate(clientCreateSchema, 'body', { logAttempts: true, logSuccess: true }),
    handleCreateClient
  );
...
router.route('/:clientId/documents')
  .get(
    ...
    handleGetDocumentsForClient
  )
  .post(
    validateMultiple(
      { schema: clientIdParamSchema, source: 'params' },
      { schema: documentUploadEnhancedSchema, source: 'body', options: { logAttempts: true } }
    ),
    handleUploadDocument
  );

// Eliminar un documento específico de un cliente
router.delete('/:clientId/documents/:documentId', 
  validateMultiple(
    { schema: clientIdParamSchema, source: 'params' },
    { schema: uuidParamSchema.extend({ documentId: uuidParamSchema.shape.id }), source: 'params' }
  ),
...
router.post('/:clientId/chat', 
  validateMultiple(
    { schema: clientIdParamSchema, source: 'params' },
    { schema: chatMessageSchema, source: 'body', options: { logAttempts: true } }
  ),
  handleChat
);

router.get('/:clientId/chat/history', 
  validate(clientIdParamSchema, 'params'),
  handleGetChatHistory
);

router.get('/:clientId/ideas', 
  validate(clientIdParamSchema, 'params'),
  handleListIdeas
);

router.post('/:clientId/ideas/:ideaId/feedback', 
  validateMultiple(
    { schema: clientIdParamSchema, source: 'params' },
...
router.get('/:clientId/activity-feed', 
  validate(clientIdParamSchema, 'params'),
...
router.get('/:clientId/contacts', 
  validate(clientIdParamSchema, 'params'),
  handleListContacts
);

router.post('/:clientId/contacts', 
  validateMultiple(
    { schema: clientIdParamSchema, source: 'params' },
    { schema: contactsBatchSchema, source: 'body', options: { logSuccess: true } }
  ),
  handleUpsertContacts
); // admite crear varios

router.delete('/:clientId/contacts/:contactId', 
  validateMultiple(
    { schema: clientIdParamSchema, source: 'params' },
    { schema: uuidParamSchema.extend({ contactId: uuidParamSchema.shape.id }), source: 'params' }
  ),
  handleDeleteContact
);
...
router.delete('/:clientId/preferences', 
  validate(clientIdParamSchema, 'params'),
