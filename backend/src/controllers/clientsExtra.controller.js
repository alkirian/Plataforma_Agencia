  ...
  try {
    ...
    if (updates.name) {
      ...
      if (existErr)
        return res
          .status(400)
          .json({ success: false, message: existErr.message });
      if (existing && existing.length > 0) {
        return res
          .status(409)
          .json({
            success: false,
            message: "Ya existe un cliente con ese nombre en tu agencia.",
          });
      }
    }
...
export const handleUpsertClientUserPreference = async (req, res, next) => {
  try {
    ...
    if (!clientId)
      return res
        .status(400)
        .json({ success: false, message: "clientId requerido" });
...
export const handleUpsertContacts = async (req, res, next) => {
  try {
    ...
    if (!contacts.length)
      return res
        .status(400)
        .json({ success: false, message: "contacts requerido" });
