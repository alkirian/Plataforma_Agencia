...
import { validateData, clientSchema, clientBrandProfileSchema, clientCardColorSchema } from '../schemas/validation.js';
...
export const handleUpdateClientBrandProfile = async (req, res, next) => {
  try {
    const token = req.headers.authorization.split(' ')[1];
    const { clientId } = req.params;
    const validation = validateData(clientBrandProfileSchema, req.body);
    ...
    const updated = await updateClientBrandProfile(clientId, validation.data, token);
    if (!updated) {
      return res.status(404).json({
        success: false,
...
export const handleUpdateClientCardColor = async (req, res, next) => {
  try {
    const token = req.headers.authorization.split(' ')[1];
    const { clientId } = req.params;
    const validation = validateData(clientCardColorSchema, req.body);
    ...
    const updated = await updateClientCardColor(clientId, validation.data.card_color, token);
    if (!updated) {
      return res.status(404).json({
        success: false,
        message: 'Cliente no encontrado o no tienes permiso para editarlo.',
...
export const handleUpdateClient = async (req, res, next) => {
  try {
    ...
    const validation = validateData(partialClientSchema, req.body);
    ...
    const updated = await updateClient(clientId, validation.data, token, req.user.id);
    if (!updated) {
      return res.status(404).json({
        success: false,
        message: 'Cliente no encontrado o no tienes permiso para editarlo.',
