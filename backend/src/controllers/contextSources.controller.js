...
export const handleProcessDocumentSource = async (req, res, next) => {
  try {
    ...
    const { file_name, storage_path, file_type, file_size, metadata = {} } = req.body || {};
...
export const handleProcessManualSource = async (req, res, next) => {
  try {
    const { clientId } = req.params;
    const token = req.headers.authorization?.split(' ')[1];
    const { content, title, category, tags = [], importance = 'medium' } = req.body || {};
...
export const handleProcessNoteSource = async (req, res, next) => {
  try {
    const { clientId } = req.params;
    const token = req.headers.authorization?.split(' ')[1];
    const { note, title, note_type = 'general', importance = 'medium', tags = [] } = req.body || {};
