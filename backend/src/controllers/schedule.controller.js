...
export const handleCreateScheduleItem = async (req, res, next) => {
  try {
    ...
    if (!validation.success) {
      console.error("❌ Error de validación en handleCreateScheduleItem:", JSON.stringify(validation.errors, null, 2));
      console.log("📦 Datos recibidos:", JSON.stringify({ ...req.body, client_id: clientId }, null, 2));
...
export const handleUpdateScheduleItem = async (req, res, next) => {
  try {
    ...
    const updatedItem = await updateScheduleItem(
      itemId,
      clientId,
      validation.data,
      token,
