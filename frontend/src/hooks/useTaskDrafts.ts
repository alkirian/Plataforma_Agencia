...
import type { ScheduleFormData, UseTaskDraftsReturn } from '../schedule/models'

interface DraftData extends ScheduleFormData {
  timestamp: number
  version: string
}
 ...
 */
export const useTaskDrafts = (clientId: string, selectedDate: Date): UseTaskDraftsReturn => {
  ...
  const saveDraft = useCallback(
    (formData: Partial<ScheduleFormData>): void => {
  ...
  const loadDraft = useCallback((): Partial<ScheduleFormData> | null => {
    const key = getDraftKey()
    if (!key) return null
    ...
    try {
      ...
      const { timestamp, version, ...formData }: DraftData = draft
  ...
  return {
    drafts: {} as Record<string, Partial<ScheduleFormData>>, // Not implemented in original
    saveDraft: (key: string, data: Partial<ScheduleFormData>) => saveDraft(data),
    loadDraft: (key: string) => loadDraft(),
    clearDraft: (key: string) => clearDraft(),
    clearAllDrafts: cleanupOldDrafts,
  }
}
