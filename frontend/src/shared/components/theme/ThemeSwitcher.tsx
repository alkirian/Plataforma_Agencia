...
import { type ColorPaletteName, getPaletteNames } from '../../theme/colorPalettes'
...
const PalettePreview: React.FC<{
  ...
  return (
    ...
    >
      ...
      {/* Palette name */}
      <div className='text-xs text-theme-text-muted text-center font-medium'>{displayName}</div>
...
const ModeToggle: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <span className='text-sm font-medium text-theme-text-primary'>{getModeLabel()}</span>
    </motion.button>
  )
}
...
export const ThemeSwitcher: React.FC<ThemeSwitcherProps> = ({
  ...
  const { currentPaletteName, setColorPalette, availablePalettes, preferences } = useTheme()
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className='text-center'>
        <h3 className='text-lg font-semibold text-theme-text-primary mb-2'>Choose Your Theme</h3>
      ...
      {showPreview && (
        ...
        >
          <div className='flex items-center justify-between'>
            <div>
              <div className='text-sm font-medium text-theme-text-primary'>Current Theme</div>
      ...
      <div className='text-center'>
        <div className='text-xs text-theme-text-muted'>
          Animations: {preferences.animations ? 'Enabled' : 'Disabled'} • Motion:{' '}
 ...
 */
export const QuickThemeSwitcher: React.FC<{ className?: string }> = ({ className = '' }) => {
  return (
    <ThemeSwitcher
      className={className}
