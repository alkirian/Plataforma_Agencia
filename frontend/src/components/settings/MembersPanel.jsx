...
import { listMembersAndInvites, createInvitations, revokeInvitation, resendInvitation } from '../../api/invitations.js';
    ...
    <div className="rounded-xl border border-[color:var(--color-border-subtle)] bg-surface-strong p-6 shadow-lg space-y-6">
      ...
      <form onSubmit={handleInvite} className="space-y-3">
        <div className="flex gap-2 flex-col sm:flex-row">
          ...
          <Button type="submit" disabled={submitting}>{submitting ? 'Invitando…' : 'Invitar'}</Button>
        </div>
      </form>
            ...
            <ul className="divide-y divide-[color:var(--color-border-subtle)]">
              {members.map((m) => (
                <li key={m.id} className="py-2 flex items-center justify-between">
                  ...
                  <span className="text-xs rounded bg-surface-soft px-2 py-1 border border-[color:var(--color-border-subtle)]">{m.role || 'member'}</span>
                </li>
              ))}
                ...
                <li key={i.id} className="py-2">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-text-muted">rol: {i.role} • expira: {new Date(i.expires_at).toLocaleDateString()}</div>
                    </div>
                    <div className="flex gap-2">
                      <Button size="sm" variant="secondary" onClick={() => handleResend(i.id)}>Reenviar</Button>
                      <Button size="sm" variant="secondary" onClick={() => handleRevoke(i.id)}>Revocar</Button>
                    </div>
                  </div>
                </li>
              ))}
