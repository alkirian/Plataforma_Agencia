...
export const ColorPaletteWidget = ({
  ...
  return (
    <div className='rounded-2xl border border-border-subtle bg-surface-strong/40 p-3 space-y-2.5 relative overflow-hidden flex-shrink-0 shadow-sm'>
      ...
      <div className='flex items-center justify-between gap-4'>
        <div className='text-left space-y-0.5'>
          ...
          <p className='text-[9px] text-text-muted leading-none'>
            Colores visuales del negocio. Haz clic en un color para copiar su código Hex.
          </p>
        </div>
        ...
        {displayPalette.map((color, idx) => (
          ...
          >
            ...
            <button
              ...
              title={`Copiar Hex: ${color}`}
            >
              {/* Copied indicator tooltip show on hover */}
            ...
            {/* Manual delete button overlay (only show if colorPalette is not empty and not analyzing) */}
            {colorPalette.length > 0 && !isAnalyzing && (
              <button
                type='button'
                onClick={() => onRemoveColor(color)}
                className='absolute -top-1 -right-1 h-3.5 w-3.5 rounded-full bg-red-500 text-white text-[8px] font-black flex items-center justify-center border border-black hover:bg-red-600 transition-all opacity-0 group-hover:opacity-100 shadow-md active:scale-90'
