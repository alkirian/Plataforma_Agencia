...
export const Breadcrumbs = ({ items, className = '' }) => {
  ...
  const autoItems = useMemo(() => {
    ...
    if (segments.length === 0)
      return [{ label: 'Dashboard', href: '/dashboard', icon: <Home size={14} /> }]
    ...
    <nav aria-label='Breadcrumb' className={`breadcrumbs-dark ${className}`}>
      <ol className='breadcrumbs-list' role='list'>
        {autoItems.map((item, idx) => {
          const isLast =
            idx === autoItems.length - 1 || (!item.href && idx === autoItems.length - 1)
          const content = (
            <span className='breadcrumbs-pill-inner'>
              ...
              <span className='breadcrumbs-label' title={item.label}>
                {item.label}
              </span>
            </span>
          )
          ...
          return (
            ...
            >
              {isLast || !item.href ? (
                <span
                  aria-current='page'
                  className={`breadcrumbs-pill ${isLast ? 'is-active' : ''} ${item.className || ''}`}
                >
                  {content}
                </span>
              ) : (
