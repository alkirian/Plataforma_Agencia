...
export const DeliverableGallery = ({ clientId }) => {
  ...
  const handleDeleteAsset = async (assetId, fileName) => {
    if (!window.confirm(`¿Estás seguro de que quieres eliminar el entregable "${fileName}"? Se quitará de la publicación del calendario.`)) {
      return;
    }
  ...
  const filteredAssets = useMemo(() => {
    return assets.filter(asset => {
      ...
      else if (filterType === 'other') matchesType = !mime.startsWith('image/') && !mime.startsWith('video/');
  ...
  const stats = useMemo(() => {
    ...
    let approved = assets.filter(a => a.schedule_items?.status === 'Aprobado' || a.schedule_items?.status === 'Publicado').length;
  ...
  const getChannelBadgeStyles = (channel = '') => {
    ...
    if (ch.includes('instagram')) return 'bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 text-white font-semibold';
    ...
    if (ch.includes('twitter') || ch.includes('x')) return 'bg-neutral-900 border border-white/20 text-white';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-surface-soft border border-white/10 rounded-xl p-4 flex flex-col justify-between">
          <span className="text-xs text-text-muted font-medium uppercase tracking-wider">Entregables Totales</span>
          <span className="text-2xl font-bold text-text-primary mt-2">{stats.total}</span>
        </div>
        <div className="bg-surface-soft border border-white/10 rounded-xl p-4 flex flex-col justify-between">
          <span className="text-xs text-text-muted font-medium uppercase tracking-wider">Imágenes IA / Diseños</span>
          <span className="text-2xl font-bold text-primary-400 mt-2">{stats.images}</span>
        </div>
        <div className="bg-surface-soft border border-white/10 rounded-xl p-4 flex flex-col justify-between">
          <span className="text-xs text-text-muted font-medium uppercase tracking-wider">Videos / Reels</span>
          <span className="text-2xl font-bold text-cyan-400 mt-2">{stats.videos}</span>
        </div>
        <div className="bg-surface-soft border border-white/10 rounded-xl p-4 flex flex-col justify-between">
          <span className="text-xs text-text-muted font-medium uppercase tracking-wider">Listos / Aprobados</span>
      ...
      {isLoading ? (
        <div className="py-24 text-center">
          ...
          <p className="text-text-muted text-sm">Cargando entregables y archivos de publicación...</p>
        </div>
        ...
        <div className="bg-surface-soft border border-white/10 rounded-xl p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-text-primary mb-2">No se encontraron entregables</h3>
                ...
                >
                  ...
                  <div className="relative aspect-video bg-surface-strong border-b border-white/5 overflow-hidden flex items-center justify-center">
                    {isImage && asset.preview_url ? (
                      ...
                      <div className="text-center p-6">
                        ...
                        <span className="text-xs text-text-muted truncate max-w-full block px-4">{asset.file_name}</span>
                      </div>
                    )}

                    {/* Badge Canal Social */}
                    {post.channel && (
                      <span className={`absolute top-2 left-2 text-[10px] uppercase tracking-wider rounded px-2 py-0.5 shadow-md ${getChannelBadgeStyles(post.channel)}`}>
                        {post.channel}
                      </span>
                    )}
                    ...
                    {post.status && (
                      <span className={`absolute top-2 right-2 text-[10px] rounded-full px-2 py-0.5 shadow-md ${getStatusBadgeStyles(post.status)}`}>
                        {post.status}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="mb-2">
                        <span className="text-[10px] uppercase font-bold tracking-widest text-primary-400">Publicación</span>
                        <h4 className="text-sm font-semibold text-text-primary line-clamp-1 group-hover:text-primary-300 transition-colors" title={post.title || 'Sin título'}>
                      ...
                      <div className="flex items-center space-x-2 text-[10px] text-text-muted mt-2">
                        ...
                        {post.scheduled_at ? (
                          ...
                          <span>Subido: {formatDate(asset.created_at)}</span>
                        )}
                      </div>
                    </div>
