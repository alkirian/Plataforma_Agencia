...
export const DocumentList = ({
  ...
  const handleDelete = async docData => {
    ...
    if (
      !confirm(`¿Estás seguro de que quieres eliminar "${docData?.file_name || 'este documento'}"?`)
    ) {
      return
    }
  ...
  return (
    <div className='mt-6 flow-root'>
      <ul role='list' className='-my-4 divide-y divide-[color:var(--color-border-subtle)]'>
        {documents.length === 0 && <li className='py-4 text-text-muted'>No hay documentos aún.</li>}
        {documents.map(doc => (
          <motion.li
            key={doc.id}
