...
export const CMSection = ({ clientId }) => {
  ...
  const [activeSubTab, setActiveSubTab] = useState('inbox'); // 'inbox' | 'ads'
  ...
  const handleExpandPost = (post) => {
    if (expandedPostId === post.id) {
      ...
      setBoostCampaignName(`Promoción - ${post.caption ? post.caption.substring(0, 30) : post.id}`);
      setBoostTotalBudget(15);
      setBoostDurationDays(3);
    }
  };

  const handleBoostSubmit = async (post) => {
    if (!boostCampaignName.trim()) {
      toast.error('El nombre de la campaña es requerido.');
      return;
    }
    setBoostingPostId(post.id);
    const dailyBudget = Math.round((boostTotalBudget / boostDurationDays) * 100) / 100;

    try {
      const res = await apiFetch(`/clients/${clientId}/meta-integration/boost`, {
        method: 'POST',
        body: JSON.stringify({
          postId: post.id,
      ...
      if (res?.success) {
        toast.success('¡Publicación promocionada correctamente!');
        setExpandedPostId('');
      } else {
        toast.error('No se pudo promocionar la publicación.');
      }
    } catch (err) {
      toast.error(err.message || 'Error al promocionar la publicación.');
    } finally {
      setBoostingPostId('');
    }
  };
  ...
  useEffect(() => {
    if (activeSubTab === 'posts' && integration && posts.length === 0) {
      fetchPosts();
    }
  ...
  // =========================================================================
  // EL DASHBOARD DE CM INTELIGENTE COMPLETO
  // =========================================================================
  return (
                  ...
                  {loadingThreads ? (
                    ...
                    filteredThreads.map(thread => {
                      ...
                      return (
                        ...
                        >
                          {/* Cabecera del Comentario (Siempre visible, clickeable para expandir/colapsar) */}
                          <div 
                          ...
                          >
                            ...
                            {/* Pie de tarjeta colapsada (Acción rápida) */}
                            {!isExpanded && (
                          ...
                          {/* Cuerpo Expandido: Editor e IA (Animado) */}
                          <AnimatePresence>
                            {isExpanded && (
                              <motion.div
