...
export const UrlSourceForm = ({ onSubmit, disabled = false }) => {
  ...
  const getStatusIcon = () => {
    if (isValidating) return <LoadingSpinner size='sm' variant='success' />
  ...
  return (
    <form onSubmit={handleSubmit} className='space-y-6'>
          ...
          >
            <div className='flex items-start space-x-3'>
              <div className='flex-shrink-0'>
                ...
                />
                <GlobeAltIcon className='w-8 h-8 text-green-400' style={{ display: 'none' }} />
              </div>
              <div className='flex-1 min-w-0'>
                <p className='text-sm font-medium text-green-300 truncate'>{urlPreview.title}</p>
                <p className='text-xs text-green-200/70'>{urlPreview.description}</p>
                <p className='text-xs text-green-200/50 mt-1'>{formData.url}</p>
              </div>
      ...
      {/* Title */}
      <div>
        <label className='block text-sm font-medium text-green-300 mb-2'>Título</label>
        <input
          type='text'
      ...
      <div>
        ...
        <input
          ...
          className='w-full px-4 py-3 bg-surface-strong border border-green-500/30 
                   ...
                   focus:ring-2 focus:ring-green-500/50 focus:border-transparent'
        />
        <p className='mt-1 text-xs text-green-200/50'>Separa los tags con comas</p>
      </div>
      ...
      <div className='p-4 bg-green-500/5 border border-green-500/20 rounded-lg'>
        <div className='flex items-start space-x-2'>
          ...
          <div className='text-sm text-green-200/70'>
            ...
            <p>
              El sistema extraerá automáticamente el contenido de la página para que puedas hacer
              preguntas sobre él.
            </p>
          </div>
        </div>
