  ...
  const getStatusInfo = (status) => {
    switch (status) {
      ...
      case PROCESSING_STATUS.PROCESSING:
        return {
          icon: () => <div className="animate-spin h-4 w-4 border-2 border-orange-400 border-t-transparent rounded-full" />,
          text: 'Procesando',
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start space-x-3 flex-1">
          ...
          <div className="flex-1 min-w-0">
            ...
            {source.source_type === 'url' && source.url && (
              <p className="text-xs text-text-muted/70 mt-1 truncate">
                {source.url}
              </p>
            )}
      ...
      {source.source_type === 'note' && source.priority && (
        <div className="mb-3">
          {getPriorityStars(source.priority)}
        </div>
      )}
      ...
      {/* Source type badge */}
      <div className={`absolute -top-2 -right-2 px-2 py-1 rounded-full text-xs font-medium ${colors.badge}`}>
