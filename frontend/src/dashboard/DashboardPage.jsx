...
import { SimpleDropdown } from '@shared/components/ui/SimpleDropdown'
...
export const DashboardPage = () => {
  ...
  const clientIds = React.useMemo(() => allClients.map(client => client.id), [allClients])
  ...
  const filteredAndSortedClients = React.useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter(
        client =>
          client.name?.toLowerCase().includes(term) || client.industry?.toLowerCase().includes(term)
      )
    }
  ...
  React.useEffect(() => {
    console.log('🔍 Debug - myAgencyResp:', myAgencyResp)
    console.log('🔍 Debug - user_role:', myAgencyResp?.data?.user_role)
    console.log('🔍 Debug - isAdmin:', isAdmin)
  }, [myAgencyResp, isAdmin])

  // Preferencias por usuario (colores de tarjeta por cliente)
  const { data: prefsResp } = useQuery({
    queryKey: ['client-prefs'],
  ...
  // TODO: Implement inline editing for client rename and industry change

  if (isLoading)
    return (
      <LoadingCard title='Cargando clientes...' description='Obteniendo la lista de tus clientes' />
    )
  if (isError)
    return (
      <ErrorCard
  ...
  return (
    <div className='space-y-6 sm:space-y-8 max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8'>
      ...
      <section className='flex flex-col items-center text-center gap-4 py-4 px-4 sm:px-0'>
        ...
        <div className='flex justify-center gap-3'>
          ...
          <Button
            ...
            aria-label='Invitar nuevos usuarios a la agencia'
      ...
      <div className='relative'>
        <div className='mx-auto max-w-[1600px] mb-6 h-px w-full border-t border-[color:var(--color-border-subtle)]' />
      ...
      {allClients.length > 0 && (
        <div className='mx-auto max-w-[1600px] flex items-center justify-end px-4 sm:px-0'>
          <div className='relative' ref={sortRef}>
            <Button
              variant='ghost'
              size='sm'
              onClick={() => setIsSortOpen(v => !v)}
              className='border border-[color:var(--color-border-subtle)] bg-surface-soft text-text-muted hover:text-text-primary'
            ...
            >
              <span className='hidden sm:inline'>
                Ordenar: {sortBy === 'name' ? 'Nombre' : sortBy === 'date' ? 'Fecha' : 'Industria'}
              </span>
      ...
      {allClients.length === 0 ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className='text-center py-12'>
          ...
          <p className='text-gray-500 text-sm mb-4'>Intenta con un término de búsqueda diferente</p>
          <Button
            variant='ghost'
            size='sm'
            onClick={() => navigate('/dashboard')}
            className='text-[color:var(--color-accent-blue)] hover:underline text-sm p-1'
          >
            Limpiar búsqueda
          </Button>
        </motion.div>
        ...
        <motion.div
          layout
          className='grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mx-auto max-w-[1600px] justify-center'
          role='list'
                    ...
                    {({ onClose }) => (
                      <div className='p-1'>
                        <Button
                          variant='ghost'
                          size='sm'
                          className='w-full justify-start px-3 py-2 text-gray-200 hover:bg-gray-700 hover:text-white transition-colors'
                          onClick={e => {
                            e.preventDefault()
                            e.stopPropagation()
                            onClose()
                        ...
                        <Button
                          ...
                          onClick={e => {
                            ...
                            e.stopPropagation()
                            onClose()
                            toast.info('Función de cambio de industria temporalmente deshabilitada')
                          }}
                          icon={<Tag size={16} />}
                        >
                          Cambiar industria
                          ...
                          <div className='grid grid-cols-6 gap-1'>
                            {[
                              ...
                              <Button
                                ...
                                variant='ghost'
                                size='sm'
                                className='h-6 w-6 p-0 rounded-md border border-gray-600 text-[10px] text-gray-400 hover:border-gray-500 transition-colors'
                                style={
                                  c === 'none'
                                    ? {
                                        background:
                                          'repeating-linear-gradient(45deg, rgba(255,255,255,0.06) 0 6px, rgba(0,0,0,0.06) 6px 12px)',
                                      }
                                    : { backgroundColor: c }
                                }
                                aria-label={
                                  c === 'none' ? 'Sin color (por defecto)' : `Establecer color ${c}`
                                }
                        ...
                        <button
                          className='w-full flex items-center gap-2 px-3 py-2 rounded-md text-gray-200 hover:bg-gray-700 hover:text-white transition-colors'
                          onClick={e => {
                            e.preventDefault()
                            e.stopPropagation()
                            onClose()
                        ...
                        <button
                          ...
                          onClick={e => {
                            ...
                            e.stopPropagation()
                            onClose()
                            window.open(`/clients/${client.id}`, '_blank', 'noopener,noreferrer')
                          }}
                        >
                        ...
                        {isAdmin && (
                          <>
                            <div className='my-1 h-px bg-gray-700' />
                            <button
                              className='w-full flex items-center gap-2 px-3 py-2 rounded-md text-red-400 hover:bg-red-900/50 hover:text-red-300 transition-colors'
                              onClick={e => {
                                e.preventDefault()
                                e.stopPropagation()
                                onClose()
                ...
                >
                  ...
                  >
                    ...
                    {statsMap[client.id]?.hasEvents ? (
                      ...
                      <div className='text-xs text-text-muted italic'>Sin tareas programadas</div>
                    )}
                  </div>
                </Link>
              </motion.div>
